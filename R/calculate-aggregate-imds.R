...
imd_lookup <- read.csv("data/IoD-2025-LSOA.csv") %>%
  ...
  select(lsoa_code_2021, imd_rank)
...
ward_imd <- all_lsoa_data %>%
  group_by(wd25cd, wd25nm, lad25cd, lad25nm) %>%
  summarise(
    pop_weighted_rank = sum(imd_rank * total_population_mid_2022) / sum(total_population_mid_2022),
    .groups = "drop"
  ) %>%
  mutate(
    agg_imd_rank = rank(pop_weighted_rank),
    imd25_decile = floor( 10 * (agg_imd_rank - 1) / max(agg_imd_rank) + 1),
    imd25_quintile = floor( 5 * (agg_imd_rank - 1) / max(agg_imd_rank) + 1)
  ) %>% 
  select(-pop_weighted_rank)

# 2022 Constituency 
cons22_imd <- all_lsoa_data %>%
  group_by(pcon22cd, pcon22nm) %>%
  summarise(
    pop_weighted_rank = sum(imd_rank * total_population_mid_2022) / sum(total_population_mid_2022),
    .groups = "drop"
  ) %>%
...
writexl::write_xlsx(output, "output/aggregate-imd25-lookup.xlsx")